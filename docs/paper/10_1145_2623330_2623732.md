...
- **cite** → [Stochastic Relaxation, Gibbs Distributions, and the Bayesian Restoration of Images](10_1109_tpami_1984_4767596.md) — DeepWalk uses Gibbs-sampling and stochastic-relaxation ideas as background for treating random walks on graphs as samples from local network neighborhoods.
- **cite** → [Modularity and community structure in networks](10_1073_pnas_0601602103.md) — DeepWalk cites modularity because community structure is a graph property that learned random-walk embeddings are expected to capture.
- **cite** → [ImageNet classification with deep convolutional neural networks](10_1145_3065386.md) — DeepWalk cites AlexNet as evidence that learned latent representations from large-scale neural models can outperform hand-engineered features.
- **cite** ← [node2vec](10_1145_2939672_2939754.md) — node2vec builds on DeepWalk's random-walk-plus-skip-gram framework for learning latent node representations from graphs.
- **enables** ← [Stochastic Relaxation, Gibbs Distributions, and the Bayesian Restoration of Images](10_1109_tpami_1984_4767596.md) — Gibbs sampling and stochastic relaxation introduced random-walk sampling ideas that DeepWalk applies to learn representations from graph neighborhoods.
- **enables** ← [Modularity and community structure in networks](10_1073_pnas_0601602103.md) — Network modularity framed communities as graph structure, while DeepWalk learned node embeddings from those local community-like neighborhoods.
