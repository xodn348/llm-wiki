...
- **cite** → [Human-level control through deep reinforcement learning](10_1038_nature14236.md) — AlphaGo Zero adapts deep reinforcement learning with neural value and policy estimation from DQN-style human-level control to Go self-play.
- **cite** → [ImageNet classification with deep convolutional neural networks](10_1145_3065386.md) — AlphaGo Zero uses deep convolutional neural networks whose large-scale image-recognition effectiveness was demonstrated by AlexNet on ImageNet.
- **cite** → [Deep Residual Learning for Image Recognition](10_1109_cvpr_2016_90.md) — AlphaGo Zero uses residual neural network blocks derived from ResNet to train deeper policy-value networks for Go.
- **cite** → [Mastering the game of Go with deep neural networks and tree search](10_1038_nature16961.md) — AlphaGo Zero removes human expert data from the earlier AlphaGo system while retaining the core combination of deep neural networks and tree search.
